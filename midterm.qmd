...
Note that we transformed the response time in hours by its sqaure root, to make 
the visuals more interpretable. The distribution of response times resembles a 
gamma distribution. It quickly rises and then decays exponentially. The 
distribution is clearer if we remove the largest outliers and adjust the bin 
width.
...
Next we'll look at the distributions for `response_time` across the different 
boroughs.
