...
The following reads in `data/rainfall_CP.csv` and merges it with the dataframe 
from part (a).
...
when predicting the current days SF count. We need to construct the covariates 
...
count data. However, the former is less applicable, since it assumes the 
...
There's not enough information here to tell whether a Poisson regression is 
...
Poisson regression. $\text{psuedo-}R^2 = 0.16$, which is similar to the Poisson 
result. Again, all but three predictors are relevant in the regression (the 
same three from Poisson model). The Pearson chi-square statistic from each 
regression suggests overdispersion in the data, since it is far greater than 
the degrees of freedom in both cases.

To sum up is simple terms:

+ Both regressions model the count of SF service requests with about the same 
  efficacy. Neither performs that well, but the fit is serviceable.
